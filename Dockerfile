...
# Disable wasm-opt on arm64 (native binary crashes with -Oz on Apple Silicon / arm64 Docker)
# CI runs on amd64 ubuntu-latest where wasm-opt works correctly
RUN ARCH=$(uname -m); \
    if [ "$ARCH" = "aarch64" ]; then \
        sed -i 's/data-wasm-opt="[^"]*"/data-wasm-opt="0"/' crates/orrery-ui/index.html; \
    fi; \
